...
namespace osu_NoFadeAnimation {
  class Program {
    static void Main(string[] args) {
      if (args.Length != 1) {
        WriteLine($ "Drag & drop osu!.exe onto {GetExecutingAssembly().GetName().Name}.exe");
        ReadLine();
        return;
      }
      ...
      IList < Instruction > entryMethodInstr = module.ResolveMethod(8910).Body.Instructions,
      ...
      module.Write(Combine(GetDirectoryName(path), $ "{GetFileNameWithoutExtension(path)}-patched{GetExtension(path)}"), new ModuleWriterOptions(module) {
        Logger = new DummyLogger(null)
      });
